...
class TestParallelActiveLearner:
    ...
    def configured_parallel_learner(self, parallel_learner):
        ...
        parallel_learner.active_learn_function = \
            AsyncMock(return_value="active_result")
    ...
    def test_create_sequential_learner(self, configured_parallel_learner):
        ...
        assert sequential.simulation_function == \
            configured_parallel_learner.simulation_function
        assert sequential.training_function == \
            configured_parallel_learner.training_function
        assert sequential.active_learn_function == \
            configured_parallel_learner.active_learn_function
        assert sequential.criterion_function == \
    ...
    def test_convert_to_sequential_config(self, parallel_learner):
        ...
        with patch(
            'rose.al.active_learner.LearnerConfig') as mock_learner_config:
    ...
    async def test_teach_validation_errors(self, parallel_learner):
        ...
        match="For single learner, use SequentialActiveLearner"):
        ...
        with pytest.raises(ValueError,
        match="learner_configs length must match parallel_learners"):
    ...
    async def test_teach_successful_parallel_execution(
        ...
        with patch.object(configured_parallel_learner,
        '_create_sequential_learner', return_value=mock_sequential):
            with patch.object(configured_parallel_learner,
            '_convert_to_sequential_config', return_value=None):
