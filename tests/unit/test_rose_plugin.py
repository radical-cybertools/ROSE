...
from rose.service.api.rest import RoseSession, RoseClient, PluginRose
...
class TestWorkflowLoader:
    ...
    def test_create_learner_sequential(self, sample_workflow_yaml, mock_engine):
        ...
        wf_def = WorkflowLoader.load_yaml(sample_workflow_yaml)
        learner, config = WorkflowLoader.create_learner(
            'wf.test001', wf_def, mock_engine
        )
...
class TestRoseSession:
    ...
    async def test_ensure_engine(self, rose_session):
        ...
        with patch('rose.service.api.rest.LocalExecutionBackend') as mock_backend, \
    ...
    async def test_submit_workflow(self, rose_session, sample_workflow_yaml):
        ...
        with patch.object(rose_session, '_ensure_engine', new_callable=AsyncMock), \
    ...
    async def test_list_workflows(self, rose_session):
        ...
        # Add some workflows
        rose_session._workflows['wf.001'] = Workflow(
            wf_id='wf.001', state=WorkflowState.COMPLETED
        )
        rose_session._workflows['wf.002'] = Workflow(
            wf_id='wf.002', state=WorkflowState.RUNNING
        )
