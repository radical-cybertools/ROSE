...
async def bootstrap():
    ...
    for shape in ["cubic", "trigonal", "tetragonal"]:
        merge_base = merge_preprocess(
            f"{data_dir}/base/data {shape} {nthread_tot}", base
        )
        merge_val = merge_preprocess(
            f"{data_dir}/validation/data {shape} {nthread_tot}", val
        )
        merge_test = merge_preprocess(
            f"{data_dir}/test/data {shape} {nthread_tot}", test
        )
        merge_study = merge_preprocess(
            f"{data_dir}/study/data {shape} {nthread_tot}", study
        )
    ...
    async def start():
        for acl_iter in range(4):
            ...
            if acl_iter != 0:
                ...
                for shape in ["cubic", "trigonal", "tetragonal"]:
                    merge = merge_preprocess(
                        f"{data_dir}/AL_phase_{acl_iter}/data cubic {nthread_tot}", sim
                    )
            ...
            train = training(
                ...
                --phase_idx {acl_iter} \
                --data_dir {data_dir} \
                --shared_file_dir {data_dir}",
                *simulations,
            )
            active = active_learn(
                f"--seed {seed + 3} --num_new_sample {num_al_sample} --policy uncertainty",
                simulations,
                train,
            )
            await active
