...
def train(
    ...
    if epoch % log_interval == 0:
        ...
        if rank == 0:
            print(
                f"epoch: {epoch}, Average loss_reg: {loss1_avg:15.8f}, loss_class: {loss2_avg:15.8f}, loss_tot: {loss_avg:15.8f}"
            )
...
def test(
    ...
    if epoch % log_interval == 0:
        if rank == 0:
            print(
                f"epoch: {epoch}, Average test_loss_reg: {loss_avg1:15.8f}, test_loss_class: {loss_avg2:15.8f}, test_loss_tot: {loss_avg:15.8f}"
            )
        loss_list.append(loss_avg)
...
def validation(rank, size, model, test_loader, criterion_reg, criterion_class, on_gpu):
    model.eval()
    ...
    for batch_idx, current_batch in enumerate(test_loader):
        ...
        diff0 += criterion_reg(
            y_pred_torch_regression[:, 0:3], regression_gndtruth
        ).item()
