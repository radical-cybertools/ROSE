...
def create_all_samples(
    ...
    np.random.seed(seed)
    print(f"IMPORTANT: rank = {rank}, seed for create_all_samples = {seed}!!!")
    ...
    sample_trigonal = generate_trigonal_random_sample(
        sz, a_min2, a_max2, alpha_min, alpha_max
    )
    sample_trigonal = _remove_out_of_box(
        "trigonal", sample_trigonal, trigonal_bounding_box, alpha_diff_trigonal
    )
    sample_tetragonal = generate_tetragonal_random_sample(
        sz, a_min2, a_max2, c_min, c_max
    )
...
def main():
    ...
    if len(sys.argv) != 6:
        print(sys.argv)
        sys.stderr.write(
            "Usage: python simulation_sample.py num_sample_total "
