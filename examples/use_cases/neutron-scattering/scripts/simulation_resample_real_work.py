...
def main():
    ...
    if len(sys.argv) != 6:
        print(sys.argv)
        sys.stderr.write(
            "Usage: python simulation_resample_real_work.py "
    ...
    sample_cubic = np.load(f"sample_cubic_rank_{rank}.npy")
    sample_trigonal = np.load(f"sample_trigonal_rank_{rank}.npy")
    sample_tetragonal = np.load(f"sample_tetragonal_rank_{rank}.npy")
