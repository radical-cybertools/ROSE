...
def sweep_trigonal_sample(sz_a, sz_alpha, a_min, a_max, alpha_min, alpha_max):
    ...
    a_mesh, alpha_mesh = np.meshgrid(
        a_arr.flatten(), alpha_arr.flatten(), indexing="ij"
    )
...
def create_all_samples(
    ...
    print(
        f"IMPORTANT: rank = {rank}, for cubic, sz = {sz}, start_off = {start_off}, end_off = {end_off}!!!"
    )
    ...
    assert sz_a >= 1, (
        "Each thread only has little a_grid to do, need different algorithm!"
    )
    diff = (a_max2 - a_min2) / (sz_a * size - 1)
    start_off = a_min2 + diff * sz_a * rank
    end_off = a_min2 + diff * (sz_a * (rank + 1) - 1)
    print(
        f"IMPORTANT: rank = {rank}, for trigonal/tetragonal, sz_a = {sz_a}, sz_dim2 = {sz_dim2}, start_off = {start_off}, end_off = {end_off}!!!"
    )

    sample_trigonal = sweep_trigonal_sample(
        sz_a, sz_dim2, start_off, end_off, alpha_min, alpha_max
    )
    sample_trigonal = _remove_out_of_box(
        "trigonal", sample_trigonal, trigonal_bounding_box, alpha_diff_trigonal
    )

    sample_tetragonal = sweep_tetragonal_sample(
        sz_a, sz_dim2, start_off, end_off, c_min, c_max
    )
...
def main():
    ...
    if len(sys.argv) != 5:
        print(sys.argv)
        sys.stderr.write(
            "Usage: python simulation_sweep.py num_sample_total "
