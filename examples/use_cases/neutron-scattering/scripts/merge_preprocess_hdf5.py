...
def main():
    if len(sys.argv) != 4:
        print(sys.argv)
        sys.stderr.write(
            "Usage: python merge_preprocess_hdf5.py dir_path sym_class num_rank"
        )
        sys.exit(0)
    dir_path = sys.argv[1]
    ...
    for rank in range(num_rank):
        filename = "".join(
            [dir_path, "/", filename_prefix, "_part", str(rank), ".hdf5"]
        )
    ...
    if sym_class == "trigonal":
        ...
        all_parameters = np.concatenate(
            [all_parameters, parameters_a, class_label], axis=1
        )
        ...
        class_label = np.tile(class_label, (all_parameters.shape[0], 1))
        all_parameters = np.concatenate(
            [all_parameters, parameters_alpha, class_label], axis=1
        )
    elif sym_class == "cubic":
        parameters_a = all_parameters[:, 0].reshape(-1, 1)
        parameters_alpha = np.ones((all_parameters.shape[0], 1)) * 90.0
        ...
        class_label = np.tile(class_label, (all_parameters.shape[0], 1))
        all_parameters = np.concatenate(
            [parameters_a, parameters_a, parameters_alpha, class_label], axis=1
        )
