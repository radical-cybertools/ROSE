...
def generate_gaussian_sample_separate(
    y_study, freq, std_dev_cubic, std_dev_trigonal, std_dev_tetragonal, do_print=False
):
    samples_cubic = []
    samples_trigonal = []
    samples_tetragonal = []
            ...
            if y_study[i, 3] == 0:  # trigonal
                for _ in range(freq[i]):
                    sample = np.random.normal(
                        loc=[y_study[i, 0], y_study[i, 2]], scale=std_dev_trigonal
                    )
                    if do_print:
                ...
                for _ in range(freq[i]):
                    sample = np.random.normal(
                        loc=[y_study[i, 0], y_study[i, 1]], scale=std_dev_tetragonal
                    )
                    if do_print:
    ...
    return (
        np.vstack(samples_cubic),
        np.vstack(samples_trigonal),
        np.vstack(samples_tetragonal),
    )


def _remove_out_of_box(sym, sample_in, bounding_box, min_diff):
    assert (
        bounding_box.shape[1] == 2
    )  # [[begin1, end1], [begin2, end2], [begin3, end3], ...]
    if sym == "cubic":
        assert sample_in.shape[1] == 1
        assert bounding_box.shape[0] == 1  # on dim a
        idx = (sample_in[:, 0] >= bounding_box[0, 0]) & (
            sample_in[:, 0] <= bounding_box[0, 1]
        )
        ...
        idx = (
            (sample_in[:, 0] >= bounding_box[0, 0])
            & (sample_in[:, 0] <= bounding_box[0, 1])
            & (sample_in[:, 1] >= bounding_box[1, 0])
            & (sample_in[:, 1] <= bounding_box[1, 1])
            & (abs(sample_in[:, 1] - 90) > min_diff)
        ...
        idx = (
            ...
            & (sample_in[:, 1] >= bounding_box[1, 0])
            & (sample_in[:, 1] <= bounding_box[1, 1])
            & (abs(sample_in[:, 0] - sample_in[:, 1]) > min_diff)
        )  # two edge should be different
        sample_in = sample_in[idx]
    else:
        exit(
            f"Error! Unrecognized sym argument = {sym} in _remove_out_of_box function call"
        )
    return sample_in
...
def create_all_samples(
    ...
    freq,
    cubic_bounding_box,
    trigonal_bounding_box,
    alpha_diff_trigonal,
    tetragonal_bounding_box,
    ac_diff_tetragonal,
):
    np.random.seed(seed_g)
    do_print = rank == 0
    print(
        f"IMPORTANT: In resample, rank = {rank}, seed for create_all_samples = {seed_g}!!!"
    )

    sample_cubic, sample_trigonal, sample_tetragonal = (
        generate_gaussian_sample_separate(
            y_study,
            freq,
    ...
    sample_trigonal = _remove_out_of_box(
        "trigonal", sample_trigonal, trigonal_bounding_box, alpha_diff_trigonal
    )
    sample_tetragonal = _remove_out_of_box(
        "tetragonal", sample_tetragonal, tetragonal_bounding_box, ac_diff_tetragonal
    )

    return (
        sample_cubic[rank::size][:],
        sample_trigonal[rank::size][:],
        sample_tetragonal[rank::size][:],
    )
...
def _sim_impl(rank, size, sym, conffile_name, sample):
    ...
    hist = gpx.add_simulated_powder_histogram(
        name + "TOFsimulation", instprm, Tmin, Tmax, Tstep, phases=gpx.phases()
    )
    hist.SampleParameters["Scale"][0] = 1000.0
    # Set to no-background
    hist["Background"][0][3] = 0.0

    symmetry = gParameters["symmetry"]
    assert symmetry == sym, f"symmetry = {symmetry} and sym = {sym}"
    ...
    if symmetry == "cubic":
        ...
        exit(f"Do not recognize symmetry of {symmetry}")

    # Distribute computation
    nsim, histosz = su.grid_sample(
        rank, size, sweepf_, sample, path_out, name_out + "_" + symmetry
    )
    end = time.time()
    print("----------------------------------------------------------")
    print(
        f"Rank = {rank}, Number of simulations ({symmetry}): {nsim}, size of histogram: {histosz}, cost {end - start} seconds"
    )
...
def main():
    ...
    if len(sys.argv) != 8:
        print(sys.argv)
        sys.stderr.write(
            "Usage: python simulation_resample.py "
    ...
    size = comm.Get_size()
    rank = comm.Get_rank()
    print(f"Rank = {rank} out of size = {size}")
    ...
    y_study = np.concatenate(
        [y_study_cubic, y_study_trigonal, y_study_tetragonal], axis=0
    )

    freq = np.load("AL-freq.npy")

    sample_cubic, sample_trigonal, sample_tetragonal = create_all_samples(
        rank,
        size,
        global_seed,
        y_study,
