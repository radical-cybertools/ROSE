...
parser.add_argument(
    "--lr", type=float, default=0.0005, help="learning rate (default: 0.0005)"
)
...
parser.add_argument("--do_streaming", action="store_true", help="Enable streaming mode")
...
def print_memory_usage(info):
    util.print_memory_usage_template(f"train_step2, phase_idx = {args.phase_idx}", info)
...
print(f"DDP: I am worker size = {size}, rank = {rank}, local_rank = {local_rank}")
...
torch.distributed.init_process_group(
    backend=backend,
    init_method=f"file://{args.shared_file_dir}/sharedfile",
    world_size=size,
    rank=rank,
)
print(
    f"Setting process group, is_initialized = {torch.distributed.is_initialized()}, nccl_avail = {torch.distributed.is_nccl_available()}, get_rank = {torch.distributed.get_rank()}, get_size = {torch.distributed.get_world_size()}"
)

if args.cuda:
    # DDP: pin GPU to local rank.
    print(
        f"rank = {rank}, local_rank = {local_rank}, num_of_gpus = {torch.cuda.device_count()}"
    )
    ...
    if torch.cuda.device_count() == 1:
        ...
        torch.cuda.set_device(
            torch.cuda.device_count() - 1 - int(local_rank)
        )  # handles Polaris NUMA topology

if (not args.cuda) and (args.num_threads != 0):
    torch.set_num_threads(args.num_threads)
print(
    f"Rank = {rank}",
    " Torch Thread setup with number of threads: ",
    torch.get_num_threads(),
    " with number of inter_op threads: ",
...
base_trigonal_file = os.path.join(
    args.data_dir, "base/data/trigonal_1522004_trigonal.hdf5"
)
base_tetragonal_file = os.path.join(
    args.data_dir, "base/data/tetragonal_1531431_tetragonal.hdf5"
)
test_cubic_file = os.path.join(args.data_dir, "test/data/cubic_1001460_cubic.hdf5")
test_trigonal_file = os.path.join(
    args.data_dir, "test/data/trigonal_1522004_trigonal.hdf5"
)
test_tetragonal_file = os.path.join(
    args.data_dir, "test/data/tetragonal_1531431_tetragonal.hdf5"
)
val_cubic_file = os.path.join(args.data_dir, "validation/data/cubic_1001460_cubic.hdf5")
val_trigonal_file = os.path.join(
    args.data_dir, "validation/data/trigonal_1522004_trigonal.hdf5"
)
val_tetragonal_file = os.path.join(
    args.data_dir, "validation/data/tetragonal_1531431_tetragonal.hdf5"
)


x_train, y_train = util.create_numpy_data(
    base_cubic_file, base_trigonal_file, base_tetragonal_file
)
x_val, y_val = util.create_numpy_data(
    val_cubic_file, val_trigonal_file, val_tetragonal_file
)
x_test, y_test = util.create_numpy_data(
    test_cubic_file, test_trigonal_file, test_tetragonal_file
)
...
x_train_torch = x_train_torch.reshape(
    (x_train_torch.shape[0], 1, x_train_torch.shape[1])
)
...
if args.do_preprocess_study:
    study_cubic_file = os.path.join(
        args.data_dir, "study/data/cubic_1001460_cubic.hdf5"
    )
    study_trigonal_file = os.path.join(
        args.data_dir, "study/data/trigonal_1522004_trigonal.hdf5"
    )
    study_tetragonal_file = os.path.join(
        args.data_dir, "study/data/tetragonal_1531431_tetragonal.hdf5"
    )
    x_study, y_study = util.create_numpy_data(
        study_cubic_file, study_trigonal_file, study_tetragonal_file
    )
    ...
    x_study_torch = x_study_torch.reshape(
        (x_study_torch.shape[0], 1, x_study_torch.shape[1])
    )
...
def criterion_reg(y_pred, y_true):
    ...
    l2_diff = (
        torch.sum((y_true - y_pred_value) ** 2, axis=1, keepdims=True)
        / torch.exp(logsig2)
        + logsig2
    )
...
if args.phase_idx > 0:
    ...
    for i in range(1, args.phase_idx + 1):
        AL_cubic_file = os.path.join(
            args.data_dir, f"AL_phase_{i}/data/cubic_1001460_cubic.hdf5"
        )
        AL_trigonal_file = os.path.join(
            args.data_dir, f"AL_phase_{i}/data/trigonal_1522004_trigonal.hdf5"
        )
        AL_tetragonal_file = os.path.join(
            args.data_dir, f"AL_phase_{i}/data/tetragonal_1531431_tetragonal.hdf5"
        )
        x_AL_temp, y_AL_temp = util.create_numpy_data(
            AL_cubic_file, AL_trigonal_file, AL_tetragonal_file
        )
    ...
    if args.do_streaming:
        for i in range(1, args.phase_idx):
            stream_cubic_file = os.path.join(
                args.data_dir, f"stream_phase_{i}/data/cubic_1001460_cubic.hdf5"
            )
            stream_trigonal_file = os.path.join(
                args.data_dir, f"stream_phase_{i}/data/trigonal_1522004_trigonal.hdf5"
            )
            stream_tetragonal_file = os.path.join(
                args.data_dir,
                f"stream_phase_{i}/data/tetragonal_1531431_tetragonal.hdf5",
            )
            x_AL_temp, y_AL_temp = util.create_numpy_data(
                stream_cubic_file, stream_trigonal_file, stream_tetragonal_file
            )
            x_AL_list.append(x_AL_temp)
            y_AL_list.append(y_AL_temp)

    for i in range(len(x_AL_list)):
        x_train_torch = torch.cat(
            (
                x_train_torch,
                torch.from_numpy(x_AL_list[i])
                .float()
                .reshape((x_AL_list[i].shape[0], 1, x_AL_list[i].shape[1])),
            ),
            axis=0,
        )
        y_train_torch = torch.cat(
            (y_train_torch, torch.from_numpy(y_AL_list[i]).float()), axis=0
        )
...
print_from_rank0(
    f"train script, before real train takes {time.time() - script_start_time}"
)
...
for epoch in range(0, args.epochs):
    ...
    print_from_rank0(f"epoch {epoch}, train takes {epoch_time - epoch_time_tot}")
    ...
    print_from_rank0(f"epoch {epoch}, validation takes {epoch_time}")
    ...
    print_from_rank0(f"epoch {epoch} takes {time.time() - epoch_time_tot}")

if rank == 0:
    torch.save(best_checkpoint, "ckpt.pth")
print_from_rank0(f"Best val loss = {best_loss} at epoch = {best_epoch}")
time_real_train = time.time() - time_real_train
print_from_rank0(f"Total training time = {time_real_train}")
...
l2_diff, sigma2, class_loss = ker.validation(
    rank,
    size,
    model=model,
    test_loader=test_loader,
    criterion_reg=criterion_l2,
    criterion_class=criterion_class,
    on_gpu=args.cuda,
)

print_from_rank0(f"Final testing time = {time.time() - st}")
