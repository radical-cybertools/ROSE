...
def acl(input_file="train_output.pkl", output_file="acl_output.pkl"):
    ...
    uncertainty = np.abs(
        y_pred_unlabeled - np.mean(y_labeled)
    )  # Example of uncertainty: deviation from mean label
    ...
    y_selected = (
        2 * X_selected + 1 + np.random.normal(0, 0.1, X_selected.shape)
    )  # Simulate labels for selected data
