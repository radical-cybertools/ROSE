...
def acl(input_file='train_output.pkl', output_file='acl_output.pkl'):
    ...
    print(f"Original shapes - X_labeled: {X_labeled.shape}, y_labeled: {y_labeled.shape}, X_unlabeled: {X_unlabeled.shape}")
    ...
    if y_labeled.shape[1] == X_labeled.shape[1]:
        ...
        y_selected = np.mean(2 * X_selected + 1, axis=1, keepdims=True) + np.random.normal(0, 0.1, (X_selected.shape[0], 1))
    ...
    print(f"Final shapes before stacking - X_labeled: {X_labeled.shape}, X_selected: {X_selected.shape}")
    print(f"Final shapes before stacking - y_labeled: {y_labeled.shape}, y_selected: {y_selected.shape}")
