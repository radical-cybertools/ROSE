...
async def uq_learner():
    ...
    async def simulation(*args, **kwargs):
        ...
        return f"{code_path}/simulation.py --train_batch {train_batch} --learner_name {learner_name} --home_dir {home_dir}"  # noqa: E501
    ...
    async def training(*args, **kwargs):
        ...
        home_dir = kwargs.get("--home_dir")
        return f"{code_path}/training.py --model_name {model_name} --learner_name {learner_name} --epochs {epochs} --home_dir {home_dir}"  # noqa: E501
    ...
    async def prediction(*args, **kwargs):
        ...
        home_dir = kwargs.get("--home_dir")
        return (
            f"{code_path}/predict.py --model_name {model_name} "
    ...
    @learner.uncertainty_quantification(
        uq_metric_name=UQ_METRIC_NAME, threshold=UQ_THRESHOLD, query_size=UQ_QUERY_SIZE
    )
    async def check_uq(*args, **kwargs):
        ...
        return (
            f"{code_path}/check_uq.py --learner_name {learner_name} --predict_dir {predict_dir} "
    ...
    for PIPELINE in PIPELINES:
        learner_configs[PIPELINE] = UQLearnerConfig(
            ...
            criterion=TaskConfig(kwargs={"--prediction_dir": f"{PIPELINE}_prediction"}),
