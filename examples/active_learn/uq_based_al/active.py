...
def acl(input_file='train_output.pkl', output_file='acl_output.pkl'):
    ...
    uncertain_indices, uq_metric = uq.select_top_uncertain(y_pred_unlabeled, k=10, metric=PREDICTIVE_ENTROPY)
    ...
    print(f"Active Learning completed. MSE: {mse:.4f} UQ: {np.mean(uq_metric):.4f}")
