    ...
    async def teach(
        ...
        async def _traininig_block(
            ...
            try:
                ...
                print(
                    f"[{self.learner_name}-{model_name}] Completed with {iteration_count} iterations "
                )
                return await asyncio.gather(*prediction_tasks)

            except Exception as e:
                print(
                    f"[{self.learner_name}-{model_name}] Failed train/prediction with error: {e}"
                )
                raise
            ...
            try:
                ...
                for i in iteration_range:
                    ...
                    if self.uncertainty_function:
                        ...
                        uq_model_stop, uq_stop_value = self._check_stop_criterion(
                            uq_value
                        )
                        ...
                        if uq_model_stop:
                            ...
                            print(
                                f"[Learner {self.learner_name}] UQ value reached its threshold - Stopping training for all models at iteration {i} with value: {uq_stop_value}"
                            )
                            break
                    ...
                    acl_task = self._register_task(
                        acl_config, deps=(training_tasks, uq_task)
                    )
                    ...
                    if self.criterion_function:
                        ...
                        if should_stop == len(stops):
                            print(
                                f"[Learner {self.learner_name}] Stopping criterion met for all models at iteration {i} with value: {stop_value}"
                            )
                            break
                ...
                print(
                    f"[Learner {self.learner_name}] Completed with {iteration_count} iterations"
                )
...
class ParallelUQLearner(UQLearner):
    ...
    def _create_sequential_learner(self, learner_name: str) -> UQLearner:
