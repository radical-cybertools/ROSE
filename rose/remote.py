...
def _read_iri_token(endpoint: str) -> str:
    ...
    if not path.exists():
        raise RuntimeError(f"IRI token file missing: {path}  (put your IRI bearer token there)")
...
def _launch_iri_or_sfapi(rt, target: TargetConfig, broker_url: str) -> dict[str, Any]:
    ...
    if target.tunnel == "forward":
        if not target.login_host:
            raise RuntimeError("remote.target.login_host is required when tunnel='forward'")
    ...
    if target.constraint:
        attrs["constraint"] = target.constraint
    if target.reservation:
        attrs["reservation"] = target.reservation
    ...
    print(
        f"[rose --remote] submitting {target.kind} job "
...
def _launch_psij(
    rt, target: TargetConfig, broker_url: str, embedded: bool = False
) -> dict[str, Any]:
    if embedded:
    ...
    attrs: dict[str, Any] = {"duration": target.walltime_min * 60, "account": target.account}
    ...
    print(
        f"[rose --remote] submitting psij job via {edge_name} "
...
def _wait_for_endpoint(
    rt, name: str, timeout: float = 30 * 60, poll: float = 3.0, heartbeat: float = 30.0
) -> None:
    ...
    while time.time() - start < timeout:
        ...
        if time.time() - last_beat >= heartbeat:
            ...
            print(f"[rose --remote] ...{elapsed}s elapsed, {int(timeout - elapsed)}s left")
            last_beat = time.time()
...
def _teardown(created: dict[str, Any] | None) -> None:
    if not created:
        return
    ...
    try:
        ...
        print(f"[rose --remote] could not cancel {created['kind']} job {created['job_id']}: {exc}")

    if created["kind"] in ("iri", "sfapi"):
        try:
            created["connect_client"].disconnect(created["endpoint_key"])
            print(
                f"[rose --remote] disconnected {created['kind']} endpoint {created['endpoint_key']}"
            )
        except Exception as exc:
            print(f"[rose --remote] could not disconnect {created['endpoint_key']}: {exc}")
...
async def run_remote(spec: WorkflowSpec) -> None:
    """Bootstrap the ``remote.target`` from *spec*, run the workflow against it, then tear the
    bootstrap job down.
    ...
    if target is None:
        raise RuntimeError("run_remote() requires 'remote.target' to be set in the spec")
    ...
    try:
        if target.kind == "psij":
            ...
            created = _launch_psij(rt, target, broker_url, embedded=remote_cfg.embedded)
        else:
