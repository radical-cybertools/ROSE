...
class WorkflowLoader:
    ...
    def load_yaml(path: str) -> dict[str, Any]:
        """Load YAML file (mocking yaml load with json for now or basic parsing if yaml lib not
        avail?

        User environment might have PyYAML. Assuming yaml is available or using json for simplicity
        if needed. The user request says 'workflow.yaml', so we should try to support YAML. If
        PyYAML is not installed, we might fallback or error. Standard python doesn't have yaml.
        """
        ...
        # But to be safe and depend only on stdlib as requested ("standard libraries only" was for
        # IPC, but let's stick to it), check if yaml module exists, otherwise parse as JSON.
    ...
    async def _handle_submit(self, req_id: str, payload: dict[str, Any]):
        ...
        try:
            ...
            task = asyncio.create_task(
                self._run_learner(wf, wf_def.get("config", {}), initial_l_config)
            )
    ...
    async def _run_learner(
        ...
        try:
            ...
            if isinstance(wf.learner_instance, ParallelActiveLearner):
                parallel_learners = learner_cfg.get(
                    "parallel_learners", workflow_def.get("parallel_learners", 2)
                )
                ...
                async for state in wf.learner_instance.start(
                    ...
                    logger.info(
                        f"Workflow {wf.wf_id} - Learner {learner_id},"
                        f" iteration {iteration} completed (metric: {metric})"
                    )
                    self._update_registry()
                logger.info(
                    f"Workflow {wf.wf_id} - Parallel execution of"
                    f" {parallel_learners} learners finished"
                )
            else:
                # SequentialActiveLearner or other async iterator
                async for state in wf.learner_instance.start(
                    max_iter=max_iter, initial_config=initial_l_config
                ):
                    wf.stats = state.to_dict()
                    logger.info(
                        f"Workflow {wf.wf_id} - Iteration {state.iteration}"
                        f" completed (metric: {state.metric_value})"
                    )
                    self._update_registry()
